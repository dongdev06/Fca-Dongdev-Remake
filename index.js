...
global.Fca = new Object({
    ...
    Require: new Object({
        ...
        Database: require("synthetic-dongdev-database")
    }),
                    ...
                    if (global.Fca.Require.FastConfig.AutoUpdate == true) { 
                        log.warn("[ FCA-DONGDEV ] •",Language.AutoUpdate);
                            try {
                                ...
                                try {
                                    require.resolve('dongdev-fca-sp');
                                }
                                catch (e) {
                                    logger.Normal(Language.InstallSupportTool);
                                        execSync('npm install dongdev-fca-sp@latest', { stdio: 'inherit' });
                                    process.exit(1);
                                }
                                    var fcasp = require('dongdev-fca-sp');
                                try {
                                    fcasp.onError()
                                }
                                catch (e) {
...
function setUserNameAndPassWord() {
    ...
    let localbrand2 = JSON.parse(readFileSync('./node_modules/fca-dongdev-remake/package.json')).version;
    console.clear();
